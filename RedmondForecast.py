...
def RedmondForecast():
    ...
    querystring = {"q":"redmond,us","lat":"47.67","lon":"-122.12","cnt":"7","units":"imperial","zip":"98053"}            
    ...
    for i in range(len(forecast)): #NOTE cycling through a loop and grabbing each of the variables I want.
        ...
        'Day' : datetime.fromtimestamp(forecast[i]['dt']).strftime('%m-%d-%y'), #NOTE grabs from existing dictionary (dt) and then converts to datetime, and then converts to a string in my format (m d y), and recreates it as "day"
        ...
        'Day Temp' : forecast[i]['temp']['day'], #NOTE grabbing values from existing dictionary
